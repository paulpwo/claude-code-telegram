...
class GitIntegration:
    ...
    async def push_branch_with_pat(
        self,
        repo_path: Path,
        branch_name: str,
        pat: str,
        ...
        """Push a local branch to GitHub using an embedded PAT in the remote URL.

        Uses https://<pat>@github.com/<owner>/<repo>.git as the push URL so no
        SSH key is needed. The PAT is never written to disk.

        Args:
            repo_path:   Path to the local git repository (must be within approved_dir).
            branch_name: Name of the branch to push.
            pat:         Plaintext GitHub PAT (decrypted by the caller).
            remote_url:  Base HTTPS remote URL (e.g. "https://github.com/owner/repo").

        Raises:
            SecurityError: repo_path outside approved directory or branch is protected.
            GitError:      git push fails.
        """
        # Block pushes to protected branches
        protected: Set[str] = set(getattr(self.settings, "git_protected_branches", []))
        protected.update(getattr(self.settings, "sdd_protected_branches", []))
        if branch_name in protected:
            raise SecurityError(f"Cannot push to protected branch: {branch_name}")

        # Validate remote URL scheme
        if not remote_url.startswith("https://"):
            raise GitError(
                f"Only HTTPS remote URLs supported for PAT push, got: {remote_url}"
            )

        # Embed PAT in URL
        auth_url = remote_url.replace("https://", f"https://{pat}@", 1)

        # Validate repo_path is within approved directory
        try:
            resolved = repo_path.resolve()
            resolved.relative_to(self.approved_dir.resolve())
        except ValueError:
            raise SecurityError("Repository path is outside the approved directory")
        ...
        try:
            ...
            process.kill()
            await process.communicate()
            raise GitError("git push timed out after 60 seconds")

        if process.returncode != 0:
            # Sanitize PAT from error message before raising
            err_text = stderr.decode(errors="replace").replace(pat, "***")
            raise GitError(f"git push failed: {err_text}")

        logger.info(
            "Branch pushed with PAT",
            branch=branch_name,
            repo=str(resolved),
        )
    ...
    async def create_pr(
        ...
        """Create a GitHub PR after the SDD branch is pushed.

        Returns the PR HTML URL on success.
        """
        from ...api.github_client import GitHubClient

        title = f"SDD: {issue_title}"
        body = (
            f"Closes #{issue_number}\n\n"
            "This branch was automatically generated by the SDD bot based on the issue above."
        )
